...
;(function() {
  ...
  var regex = {
    ...
    dialogue: /^((?!!)[\wÀ-ÖØ-öø-ÿ*_]+[0-9A-Z (._\-')]*)(\^?)?(?:\n(?!\n+))([\s\S]+)/,
    parenthetical: /^(\(.+\))$/,

    action: /^(.+)/g,
    centered: /^(?:(&gt;) *)(.+)(?: *(&lt;))(\n.+)*/g,
    
    forced_transition: /^(?:(&gt;) *)(.+)([^(&]|&(?!lt;))(\n.+)*/g,
  ...
  var tokenize = function (script) {
    ...
    while (i--) {
      ...
      // forced transitions
      if (match = line.match(regex.forced_transition)) {
        tokens.push({ type: 'transition', text: match[0].replace(/^(&gt;)/g, '') });
        continue;
      }
      ...
      tokens.push({ type: 'action', text: line.replace(/^(!)/g, '') });
    }
